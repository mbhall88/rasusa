# Roadmap: Unaligned SAM/BAM/CRAM Support in `reads`
...
- [x] Implement `RecordSource` for `Alignment` (unaligned SAM/BAM/CRAM via `noodles`).

## Phase 3: Implementation (Done)
- [x] Implement `read_lengths` for unaligned SAM/BAM/CRAM.
- [x] Implement `filter_reads_into` for unaligned SAM/BAM/CRAM.
- [x] Support paired-end logic for unaligned SAM/BAM/CRAM.
- [x] Update CLI to accept unaligned SAM/BAM/CRAM extensions in the `reads` subcommand.

## Phase 4: Validation & Testing (Done)
- [x] Add unit tests for unaligned SAM/BAM/CRAM record length gathering.
- [x] Add integration tests using small unaligned SAM/BAM/CRAM test cases.
